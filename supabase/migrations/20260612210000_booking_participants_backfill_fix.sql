...
as $$
begin
  if new.status::text in ('cancelled', 'expired', 'completed', 'no_show') then
    new.is_blocking := false;
    return new;
  end if;

  if new.status::text = 'pending' and new.created_at < now() - interval '15 minutes' then
    new.is_blocking := false;
    return new;
  end if;
  ...
  if new.is_blocking is distinct from old.is_blocking then
    return new;
  end if;
...
begin
  ...
  if v_has_racket_cols then
    ...
    execute format(
      $sql$
      insert into public.booking_participants (
        booking_id, player_id, seat_index, share_price, payment_method, status, created_at
      )
      select
        b.id,
        b.%I,
