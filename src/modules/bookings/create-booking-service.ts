...
function isLikelyRpcSignatureMismatch(err: {
  message?: string;
  code?: string;
  details?: string;
  hint?: string;
...
function mapBookingRpcFailure(row: BookingRpcRow | null): BookingResult {
  ...
  if (code === "INSERT_FAILED") {
    ...
    return {
      ok: false,
      error: "Impossible d'enregistrer la réservation. Réessayez ou choisissez un autre créneau.",
      code: "SERVER_ERROR",
    };
  }
...
export async function createBookingForUser(
  ...
  try {
    ...
    console.error("[createBookingForUser] eligibility check failed", e);
    return {
      ok: false,
  ...
  if (clubErr || !clubPolicy) {
    console.error("[createBookingForUser] club read failed", clubErr?.message, clubErr);
  ...
  if (courtErr) {
    console.error("[createBookingForUser] court read failed", courtErr.message);
  }
  ...
  try {
    ...
    console.error("[createBookingForUser] pricing failed", e);
  ...
  if (useRacketRpc && rpcError && isLikelyRpcSignatureMismatch(rpcError)) {
    console.warn("[createBookingForUser] repli RPC sans raquettes", rpcError.message);
  ...
  void notifyBookingCreated({ bookingId, playerId: user.id }).catch((err) =>
    console.error("[createBookingForUser] notifyBookingCreated failed", err),
  );
