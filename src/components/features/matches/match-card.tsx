...
export function MatchCard({ match, locale = "fr" }: MatchCardProps) {
  ...
  return (
    <Link href={`/${locale}/matches/${match.id}`} className="block">
      <Card className="p-0 overflow-hidden hover:shadow-2xl hover:shadow-[var(--gold)]/20 transition-all duration-300 cursor-pointer group bg-[var(--surface)] border-[var(--border)] rounded-[2rem] hover:-translate-y-1">
      <div className="flex h-full min-h-[140px] relative">
        ...
        <div className="bg-black p-5 flex flex-col items-center justify-center min-w-[100px] text-white relative overflow-hidden border-r border-[var(--border)]">
        ...
        <div className="flex-1 p-5 flex flex-col justify-between">
            ...
            <div className="flex justify-between items-start gap-2">
              <div className="space-y-0.5">
                ...
                <div className="flex items-center gap-1 text-[10px] text-white/50 font-medium">
                  <MapPin className="h-3 w-3 text-[var(--gold)]/60" />
                  <span>Tunis, Tunisie</span>
                </div>
              </div>
              <Badge variant={slotsLeft > 0 ? "success" : "secondary"} className={cn("shrink-0 rounded-lg", slotsLeft > 0 ? "bg-[var(--gold)]/20 text-[var(--gold)] border-0" : "bg-white/10 text-white/50 border-0")}>
              ...
              <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                <div 
                  className={cn(
                    ...
                    progress === 75 ? "bg-[var(--gold)] w-3/4" :
                    progress === 50 ? "bg-[var(--gold)] w-1/2" :
                    progress === 25 ? "bg-[var(--gold)] w-1/4" : "bg-[var(--gold)] w-0"
                  )}
                />
              </div>
            </div>
          ...
          <div className="pt-3 flex justify-between items-center border-t border-white/5">
            ...
            <div className="h-8 w-8 rounded-full bg-[var(--gold)]/10 flex items-center justify-center text-[var(--gold)] group-hover:bg-[var(--gold)] group-hover:text-black transition-all">
